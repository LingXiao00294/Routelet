...
流式与非流式响应中的异常 token 用量在计费和记录时会被忽略，有效字段仍会保留；不会因此中断响应、丢弃调用记录或阻止 Provider 资源关闭。非流式响应正文保持 Provider 原样。

流式请求在首个事件预取期间被取消，或在发送响应头时断开，也会记录一次 `client_cancelled`，保留已开始的 Provider 尝试次数并释放连接与并发槽。
...
界面使用 **Providers** 管理 API 服务连接，使用 **Routers** 管理虚拟模型映射。单个对象分别称为 Provider 和 Router；客户端仍将 Router 名称作为 `model` 发送。
...
删除 Provider 或实际模型前会列出受影响 Router；确认后在同一草稿中清理引用和无效固定模型，没有剩余候选的 Router 一起移除。允许显式删除至空配置。已保存 Provider 的 API Key 留空即保留，明文密钥不写入浏览器存储；价格留空与显式 0 保持区分。发布成功但重新读取失败时会提示已保存，并清除内存中的明文密钥。

总览指标和模型排行为累计数据，趋势时间范围只作用于请求流量图，按 UTC 自然日补齐没有调用的日期。调用时间按浏览器本地时区显示。自动刷新间隔为 15 秒，隐藏页面暂停轮询，同批请求不重叠；调用筛选保存在 URL 中，过期响应不会覆盖新筛选。

`Ctrl / ⌘ + K` 打开页面、Router 和 Provider 搜索；`Ctrl / ⌘ + S` 打开发布检查；`Esc` 关闭弹窗。侧栏底部切换主题、复制当前同源 API 端点。实验室请求会产生真实 Provider 调用与费用，仅在点击发送后执行，最长等待 180 秒。
