...
| Routers | 可视化候选链、带目标位置预览和动画的拖动排序、点击候选框切换固定模型、复制 Router，以及全局自动故障转移开关 |
| Providers | API 连接、可拖动排序的模型目录、四类价格、并发 / 排队 / 超时设置，以及实时熔断重置 |
...
4. 拖动候选模型左侧的手柄调整优先级：拖动时显示目标位置，松开后以动画完成排序，按 Esc 可取消。也可用上下按钮或聚焦手柄后按上下方向键调整。选择固定模型后，「自动故障转移」开关开启时按候选顺序故障转移；关闭时仅请求固定模型，报错直接返回，不尝试备用 Provider。没有固定模型时自动选中第一个候选。开关更改也保存在草稿中，需要「检查并发布」后生效，对普通和流式请求都有效。
   在 Routers 总览点击候选模型框，也可直接切换该 Router 的固定模型。Providers 卡片的「添加模型」位于实际模型标题栏，模型左侧手柄支持拖动排序或上下方向键；目录顺序保存在草稿中，发布后仍会保留。
...
「系统设置」中的调用正文记录开关直接作用于运行时，无需发布。开启前会再次提示正文可能含敏感内容、响应记录会显著增大数据库；可选 15 分钟至 24 小时，到期或重启后自动关闭，也可立即关闭。记录开启时可重设期限，新期限从当前时间计算，可能早于原期限。关闭不会删除已有正文。
