...
请求与响应正文默认不记录；调用元数据、用量、费用和故障转移明细仍照常保存。正文记录是运行时临时开关，不写入配置，服务重启后关闭。`GET /api/recording/bodies` 返回 `{enabled, expires_at}`；`PUT /api/recording/bodies` 接受 `{"duration_minutes": 15}`，可选时长为 15、60、240 或 1440 分钟，返回自动关闭时间（UTC）；`DELETE /api/recording/bodies` 立即关闭。开启期间提交的调用会保存请求与非流式响应正文；到期自动关闭，不清理既有记录。正文可能含敏感信息，尤其响应记录会显著增加数据库大小。
