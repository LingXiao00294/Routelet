...
Routelet 不校验客户端传入的 token，Dashboard 还能读取调用详情、修改配置和重置熔断器。请求与响应正文默认不记录；在系统设置中限时开启后，`calls.db` 会保存正文（单项超过 256 KiB 时保存带 `_truncated` 标记的有界预览），其中可能包含提示词、模型输出和其他敏感数据。请限制配置文件、数据库、日志及备份的文件权限，并按自身保留策略清理。
...
启动时会为历史统计及调用列表建立覆盖索引，并将旧的虚拟模型和 Provider 索引替换为统计索引。已有大型数据库首次升级需要等待索引构建，也需要额外磁盘空间；调用记录不会因此删除。文件数据库使用 SQLite WAL 模式，读取使用独立连接以避免面板统计与写入队列相互排队。运行中可能出现 `calls.db-wal` 和 `calls.db-shm`；请停止服务后再备份或复制数据库，不要只复制运行中的主文件。

调用记录属于尽力而为的观测数据，请求响应不等待 SQLite 提交。正文记录默认关闭；临时开启期间，请求与非流式响应正文各自最多保存 256 KiB 的有效 JSON，超限内容保存为带 `_truncated`、原始字节数和文本预览的截断信封。关闭后仍会保存用量和调用元数据，也不会自动删除历史正文。
